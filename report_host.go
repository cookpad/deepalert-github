...
func buildHostInspections(hosts map[string][]deepalert.ReportHost,
	...
	for hv, contents := range hosts {
		attr, _ := attrs[hv]
		merged := mergeReportHost(contents)
		...
		nodes = append(nodes, buildReportHostBaseSection(merged)...)
		nodes = append(nodes, buildActivitiesSection(merged.Activities)...)
		nodes = append(nodes, buildReportHostDomainSection(merged.RelatedDomains)...)
...
func mergeReportHost(contents []deepalert.ReportHost) (merged deepalert.ReportHost) {
...
func buildReportHostBaseSection(merged deepalert.ReportHost) []md.Node {
	type itemSet struct {
		title string
		items []string
	}
